...
class TestRAGSystemQueryProcessing:
    ...
    def test_query_without_session(self, mock_doc_processor, mock_session_manager, 
                                 mock_ai_generator_class, mock_vector_store_class):
        ...
        mock_sources = [{"text": "ML Course - Lesson 1", "link": "http://example.com/lesson1"}]
    ...
    @patch('rag_system.SessionManager')
    @patch('rag_system.DocumentProcessor')
    def test_query_with_session(self, mock_doc_processor, mock_session_manager,
                              mock_ai_generator_class, mock_vector_store_class):
        ...
        # Mock AI response
        mock_ai_generator.generate_response.return_value = "Neural networks are a key component..."
        
        config = Config()
        rag_system = RAGSystem(config)
        
        # Mock sources
        mock_sources = [{"text": "Deep Learning Course", "link": "http://example.com/dl"}]
        rag_system.tool_manager.get_last_sources.return_value = mock_sources
        
        # Execute query with session
        session_id = "test-session-123"
        response, sources = rag_system.query("Tell me about neural networks", session_id)
    ...
    @patch('rag_system.SessionManager')
    @patch('rag_system.DocumentProcessor')
    def test_query_with_empty_sources(self, mock_doc_processor, mock_session_manager,
                                    mock_ai_generator_class, mock_vector_store_class):
        """Test query processing when no sources are found"""
        # Setup mocks
        mock_ai_generator = mock_ai_generator_class.return_value
        mock_ai_generator.generate_response.return_value = "I don't have information about that topic."
    ...
    @patch('rag_system.SessionManager')
    @patch('rag_system.DocumentProcessor')
    def test_query_with_tool_usage_simulation(self, mock_doc_processor, mock_session_manager,
                                            mock_ai_generator_class, mock_vector_store_class):
        ...
        mock_ai_generator.generate_response.return_value = "Based on course materials, machine learning involves..."
        
        config = Config()
        rag_system = RAGSystem(config)
        
        # Mock tool manager execution
        mock_sources = [{"text": "ML Fundamentals - Lesson 2", "link": "http://example.com/ml/lesson2"}]
        rag_system.tool_manager.get_last_sources.return_value = mock_sources
        
        # Execute query that should trigger tool usage
        response, sources = rag_system.query("Explain machine learning algorithms in detail")
...
class TestRAGSystemCourseManagement:
    ...
    @patch('rag_system.SessionManager')
    @patch('rag_system.DocumentProcessor')
    def test_add_course_document_success(self, mock_doc_processor_class, mock_session_manager,
                                       mock_ai_generator_class, mock_vector_store_class):
    ...
    @patch('rag_system.SessionManager')
    @patch('rag_system.DocumentProcessor')
    def test_add_course_document_exception(self, mock_doc_processor_class, mock_session_manager,
                                         mock_ai_generator_class, mock_vector_store_class):
    ...
    @patch('rag_system.SessionManager')
    @patch('rag_system.DocumentProcessor')
    def test_add_course_folder_success(self, mock_doc_processor_class, mock_session_manager,
                                     mock_ai_generator_class, mock_vector_store_class,
                                     mock_listdir, mock_path_exists):
        """Test successful course folder processing"""
        # Setup mocks
        mock_path_exists.return_value = True
        mock_listdir.return_value = ["course1.txt", "course2.txt", "readme.md"]  # Only .txt should be processed
    ...
    @patch('rag_system.SessionManager')
    @patch('rag_system.DocumentProcessor')
    def test_add_course_folder_nonexistent(self, mock_doc_processor_class, mock_session_manager,
                                         mock_ai_generator_class, mock_vector_store_class, 
                                         mock_path_exists):
...
class TestRAGSystemAnalytics:
    ...
    @patch('rag_system.SessionManager')
    @patch('rag_system.DocumentProcessor')
    def test_get_course_analytics(self, mock_doc_processor, mock_session_manager,
                                mock_ai_generator_class, mock_vector_store_class):
...
class TestRAGSystemIntegration:
    ...
    @patch('rag_system.SessionManager')
    @patch('rag_system.DocumentProcessor')
    def test_component_initialization(self, mock_doc_processor_class, mock_session_manager_class,
                                    mock_ai_generator_class, mock_vector_store_class):
        ...
        mock_vector_store_class.assert_called_once_with("test_chroma", "test-embedding-model", 5)
        mock_ai_generator_class.assert_called_once_with("test-api-key", "claude-test-model")
    ...
    @patch('rag_system.SessionManager')
    @patch('rag_system.DocumentProcessor')
    def test_tool_manager_setup(self, mock_doc_processor_class, mock_session_manager_class,
                              mock_ai_generator_class, mock_vector_store_class):
...
class TestRAGSystemConfigurationImpact:
    ...
    @patch('rag_system.SessionManager')
    @patch('rag_system.DocumentProcessor')
    def test_max_results_zero_impact_on_rag_query(self, mock_doc_processor, mock_session_manager,
                                                 mock_ai_generator_class, mock_vector_store_class):
        ...
        # Setup mocks
        mock_ai_generator = mock_ai_generator_class.return_value
        mock_ai_generator.generate_response.return_value = "I don't have information about that."
    ...
    @patch('rag_system.SessionManager')
    @patch('rag_system.DocumentProcessor')
    def test_proper_max_results_impact_on_rag_query(self, mock_doc_processor, mock_session_manager,
                                                   mock_ai_generator_class, mock_vector_store_class):
        ...
        # Setup mocks
        mock_ai_generator = mock_ai_generator_class.return_value
        mock_ai_generator.generate_response.return_value = "Based on course materials, machine learning is..."
        ...
        mock_sources = [{"text": "ML Course - Lesson 1", "link": "http://example.com/ml"}]
        rag_system.tool_manager.get_last_sources.return_value = mock_sources
